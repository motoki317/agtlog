...
func TestDetailHelpIncludesEveryRequiredBinding(t *testing.T) {
	...
	view := m.helpView()
	for _, want := range []string{"j/k scroll", "g/G edge", "space toggle", "E expand all", "C collapse all", "enter open", "tab tabs", "w wrap", "mouse wheel scroll · click select · click again activate", "esc/h back", "? help", "ctrl-c quit"} {
		if !strings.Contains(view, want) {
...
func TestDetailHelpKeepsBindingsVisibleAtFortyColumns(t *testing.T) {
	...
	for _, want := range []string{"j/k scroll", "g/G edge", "space toggle", "E expand all", "C collapse all", "enter open", "tab tabs", "w wrap", "esc/h back", "? help", "q/ctrl-c quit"} {
		if !strings.Contains(view, want) {
